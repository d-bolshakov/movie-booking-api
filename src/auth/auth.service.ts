...
export class AuthService {
    ...
    async registration(dto: RegisterUserDto): Promise<SendUserDto> {
        ...
        if (candidate) {
            throw new HttpException(`User with ${dto.email} email already exists`, HttpStatus.BAD_REQUEST);
        }
        const hashPassword = await bcryptjs.hash(dto.password, 3)
        const createdUser = await this.userRepository.create({...dto, password: hashPassword});
    ...
    async login(dto: LoginUserDto): Promise<SendUserDto> {
        ...
        if (!user) throw new HttpException(`User with ${dto.email} email not exist`, HttpStatus.BAD_REQUEST);
        const isPassValid = bcryptjs.compareSync(dto.password, user.password)
        if (!isPassValid) throw new HttpException('Invalid password', HttpStatus.BAD_REQUEST);
    ...
    async refresh(userId: number, rt: string): Promise<SendUserDto> {
        ...
        if (!user) throw new HttpException(`User with id ${userId} not exist`, HttpStatus.BAD_REQUEST);
        const storedrt = await  await this.tokenRepository.findOne({
            where: {
                user_id: userId
            }
        })
        if (!storedrt) throw new HttpException(`User with id ${userId} does not have a refresh token`, HttpStatus.BAD_REQUEST);
        if (storedrt.refresh_token !== rt) throw new HttpException(`Invalid refresh token`, HttpStatus.BAD_REQUEST);
        const tokens = await this.getTokens(user.id, user.role)
        await this.updateRefreshToken(user.id, tokens.refresh_token)
